...
if current_group:
    groups.append(copy.deepcopy(current_group)) #ensure each group is independent
...
for student in group_extra:
    ...
    for idx in target_groups:
        ...
        if len(group) < subgroup_size:
            if (gender == "Female" and female_count_extra < max_females) or \
               (gender == "Male" and male_count_extra < max_males):
                group.append(copy.deepcopy(student))
                placed = True
                break
        #adds students if not placed
    if not placed:
        for idx in target_groups:
            group = groups[idx]
            if len(group) < subgroup_size:
    ...
    while currentAttempts < randAttempts: # Attempt the random swap at n attempts
        ...
        for st1Index, st1 in enumerate(groupOne):
            for st2Index, st2 in enumerate(groupTwo):
                ...
                #gender rule check
                if not (gender_rule(groupOne) and gender_rule(groupTwo)):
